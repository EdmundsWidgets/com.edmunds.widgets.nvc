    ...
    CheckboxGroup.prototype = {
        ...
        renderList: function(){
            ...
            for (i = 0; i < _length; i = i + 1) {
                ...
                $itemEl = new Element(itemEl);
                $itemEl.addClass('features-list-item');
                itemEl.innerHTML = [
                    '<label for="for' + list[i].id + '">',
                        '<input type="checkbox" data-id="' + list[i].id + '" id="for' + list[i].id + '" value="' + _price + '" name="group_options_' + this.group + '">',
                        '<span class="feature-price">' + (list[i].included ? 'Included' : priceHtml) + '</span>',
                        '<span class="feature-name">' + list[i].name + '</span>',
                    '</label>'
                ].join('');
                _listEl.appendChild(itemEl);
            }
        ...
        template: [
            ...
            '<div class="option-wrap">',
                '<ul class="list features-list"></ul>',
